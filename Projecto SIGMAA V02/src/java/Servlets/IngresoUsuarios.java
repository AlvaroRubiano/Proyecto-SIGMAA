...
public class IngresoUsuarios extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String tipo = "";        
        String adm = "administrativo";
        String tutor = "tutor";
                
        try {
            String consulta = "Select * FROM usuarios WHERE Mail_users = '"+nombre+"' AND Pass_users='"+contrasena+"'";
            pst =(PreparedStatement) conexion.getConexion().prepareStatement(consulta);
            rs = pst.executeQuery();
                        
            while(rs.next()){
                tipo =rs.getString("Type_users");                
            }
            
            //out.print(tipo+"/"+usuario+"/"+password1);
            
            GestionesUsuarios us = new GestionesUsuarios();
            
            if(us.autenticacion(nombre, contrasena)){
                HttpSession objetoSesion = request.getSession(true);
                objetoSesion.setAttribute("UsuarioLogeado", nombre);
                if(tipo.equals(adm)){                
