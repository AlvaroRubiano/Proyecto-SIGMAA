 ...
 */
public class PdfMonitoresAcademicos extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            try {
                String consulta = "SELECT usuarios.Name_users, usuarios.Mail_users, usuarios.Phone_users "
                        + "FROM usuarios "
                        + "WHERE usuarios.Type_users=2 "
                        + "ORDER BY usuarios.Name_users ASC";
                ...
                try {
                    ...
                    PdfPCell celdaDatoReporte = new PdfPCell(new Phrase("Registro Monitores Académicos", FontFactory.getFont("Arial",10, Font.NORMAL, BaseColor.BLACK)));
                    ...
                    float[] medidaCeldas4 = {4f, 3f, 1.5f };
                    ...
                    PdfPCell celdaCorreo = new PdfPCell(new Phrase("CORREO", FontFactory.getFont("Arial",8, Font.NORMAL, BaseColor.BLACK)));
                    celdaCorreo.setVerticalAlignment(Element.ALIGN_MIDDLE);
                    celdaCorreo.setHorizontalAlignment(Element.ALIGN_CENTER);
                    
                    PdfPCell celdaCelular = new PdfPCell(new Phrase("CELULAR", FontFactory.getFont("Arial",8, Font.NORMAL, BaseColor.BLACK)));
                    celdaCelular.setVerticalAlignment(Element.ALIGN_MIDDLE);
                    celdaCelular.setHorizontalAlignment(Element.ALIGN_CENTER);

                    tabla4.addCell(celdaNombreMonitor);                            
                    tabla4.addCell(celdaCorreo);
                    tabla4.addCell(celdaCelular);
                    documento.add(tabla4);
                                
                    //Quinta parte de la tabla
                    PdfPTable tabla5 = new PdfPTable(3);
                    float[] medidaCeldas5 = {4f, 3f, 1.5f };
